...
function calculatePriority(task: Task, subjects: Subject[]): number {
	...
	return (
		(urgency * 0.5 + difficulty * 0.3 + (task.duration / 60) * 0.2) * typeWeight
	);
}
...
async function handleSignup(request: Request, env: Env): Promise<Response> {
	...
	if (!isValidName(firstName)) {
		return jsonResponse(
			{ error: "First name is required (1-100 chars)" },
			400,
			request
		);
	}
	...
	await env.CYBER_ACADEMIA_USERS.put(
		`pending_${normalizedEmail}`,
		JSON.stringify(pendingUser),
		{ expirationTtl: VERIFICATION_TTL }
	);

	await env.CYBER_ACADEMIA_SESSIONS.put(
		`verify_${normalizedEmail}`,
		verificationCode,
		{ expirationTtl: VERIFICATION_TTL }
	);
...
async function handleVerify(request: Request, env: Env): Promise<Response> {
	...
	const normalizedEmail = email.toLowerCase().trim();
	const storedCode = await env.CYBER_ACADEMIA_SESSIONS.get(
		`verify_${normalizedEmail}`
	);
	...
	const pendingUserRaw = await env.CYBER_ACADEMIA_USERS.get(
		`pending_${normalizedEmail}`
	);
	if (!pendingUserRaw) {
		return jsonResponse(
...
async function incrementLoginAttempts(
	...
	const currentAttempts =
		Number((await env.CYBER_ACADEMIA_SESSIONS.get(attemptsKey)) || 0) + 1;
